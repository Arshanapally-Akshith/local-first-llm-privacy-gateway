...
**Typed exceptions.** A hierarchy rooted at `GatewayError`, with meaningful children: `DetectionError`, `SurrogateDomainError`, `RehydrationError`, `SessionExpiredError`, `UpstreamError`, `ConfigError`. Callers catch categories, not strings. Never raise bare `Exception`.
